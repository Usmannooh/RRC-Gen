...
#
#     # Check if using Adam or SGD, and adjust the optimizer accordingly
#     if args.optim == 'Adam':
...
#             [{'params': model.visual_extractor.parameters(), 'lr': args.lr_ve},
#              {'params': ed_params, 'lr': args.lr_ed}],
#             momentum=args.momentum,  # Use momentum instead of betas
#             weight_decay=args.weight_decay,
#             nesterov=args.nesterov  # Optional: use Nesterov acceleration
#         )
#     else:
...
#     dca_ids = set(map(id, list(model.stacked_dca.parameters()) +
#                            list(model.contrastive.parameters())))
#     ed_params  = [p for p in model.parameters()
#                   if id(p) not in ve_ids and id(p) not in dca_ids]
#     dca_params = [p for p in model.parameters() if id(p) in dca_ids]
#
#     optimizer = getattr(torch.optim, args.optim)(
#         [{'params': model.visual_extractor.parameters(), 'lr': args.lr_ve},
#          {'params': dca_params,                          'lr': args.lr_dca},
#          {'params': ed_params,                           'lr': args.lr_ed}],
#         betas=args.adam_betas,
#         eps=args.adam_eps,
#         weight_decay=args.weight_decay,
