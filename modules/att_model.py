...
# CLASS  BN on att_embed input stabilizes 2048-dim features
class BatchNorm1dLastDim(nn.Module):
    """BatchNorm1d that operates on the last feature dimension.
    Handles both 2-D [B*N, D] (PackedSequence path) and 3-D [B, N, D] paths."""
    ...
    def forward(self, x):
        if x.dim() == 3:
            B, N, D = x.shape
            x = x.contiguous().reshape(B * N, D)   # contiguous() ensures safe reshape after pack/unpack
            x = self.bn(x)
            x = x.reshape(B, N, D)
            return x
        elif x.dim() == 2:
            ...
            raise ValueError(f"BatchNorm1dLastDim expects 2D or 3D input, got {x.dim()}D")
...
class AttModel(CaptionModel):
    def __init__(self, args, tokenizer):
        ...
        # BN on att_embed input stabilizes 2048-dim features
        self.att_embed = nn.Sequential(*(
                ((BatchNorm1dLastDim(self.att_feat_size),) if self.use_bn else ()) +
                (nn.Linear(self.att_feat_size, self.input_encoding_size),
                 nn.ReLU(),
                 nn.Dropout(self.drop_prob_lm)) +
                ((BatchNorm1dLastDim(self.input_encoding_size),) if self.use_bn == 2 else ())))
        # self.att_embed = nn.Sequential(*(
