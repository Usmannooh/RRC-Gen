...
class ContextAwareBiGate(nn.Module):
    ...
    def forward(self, input, context):
        forward_gate = torch.sigmoid(self.linear_forward(input))
        backward_gate = torch.sigmoid(self.linear_backward(input))
        ...
        output = input * forward_gate + context_backward * backward_gate # ye Original he
        return output
...
class MultiThreadMemory(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, topk=32):
        ...
        self.topk = topk  # Make configurable (not hardcoded)
    ...
    def forward(self, query, key, value, mask=None, layer_past=None):
        ...
        # CRITICAL: Fallback to full attention if topk >= memory size (avoids signal loss)
        if self.topk >= key.size(-2) or self.topk == 0:
            x, self.attn = attention(query, key, value, mask=mask, dropout=self.dropout)
        else:
            x, self.attn = memory_querying_responding(
                query, key, value, mask=mask, dropout=self.dropout, topk=self.topk
            )
...
class PositionwiseFeedForward(nn.Module):
    ...
    def forward(self, x):
        # REVERT TO ReLU (match original cmn training)
...
class BaseCMN(AttModel):
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask):
        # Determine the device of the memory tensor (you can choose any tensor on the correct device)
        device = memory.device

        if len(state) == 0:
            # Move it to the same device as memory
            it = it.to(device)
            ys = it.unsqueeze(1)
            past = [fc_feats_ph.new_zeros(self.num_layers * 2, fc_feats_ph.shape[0], 0, self.d_model).to(device),
                    fc_feats_ph.new_zeros(self.num_layers * 2, fc_feats_ph.shape[0], 0, self.d_model).to(device)]
        else:
            # Move it to the same device as state[0][0]
            it = it.to(state[0][0].device)
            ys = torch.cat([state[0][0], it.unsqueeze(1)], dim=1)
            past = state[1:]

        # Move other tensors involved in the decode operation to the correct device
        mask = mask.to(device)
        memory = memory.to(device)

        out, past = self.model.decode(memory, mask, ys, subsequent_mask(ys.size(1)).to(device), past=past,
                                      memory_matrix=self.memory_matrix.to(device))
